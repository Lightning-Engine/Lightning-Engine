...
CXXFLAGS = -std=c++11 -Wall -Wextra -pedantic -Og -g
LDFLAGS =
...
src_engine = $(src_dir_engine)/li_assert.c $(src_dir_engine)/li_gl.c $(src_dir_engine)/li_win.c $(src_dir_engine)/window.cc $(platform_engine_source)
...
$(bin_engine): $(obj_engine)
	@mkdir -p $(@D)
	$(CXX) $(LDFLAGS) $(platform_soname)$(notdir $@) -shared -o $@ $^ $(platform_engine_flags)
...
$(bin_win_xlib): $(obj_win_xlib) $(bin_engine)
	@mkdir -p $(@D)
	$(CXX) $(LDFLAGS) $(platform_soname)$(notdir $@) -shared -o $@ $^ -lX11 -lGL

$(bin_win_win32): $(obj_win_win32) $(bin_engine)
	@mkdir -p $(@D)
	$(CXX) $(LDFLAGS) $(platform_soname)$(notdir $@) -shared -o $@ $^ -lgdi32 -lopengl32

$(bin_win_cocoa): $(obj_win_cocoa) $(bin_engine)
	@mkdir -p $(@D)
	$(CXX) $(LDFLAGS) $(platform_soname)$(notdir $@) -shared -o $@ $^ -framework Cocoa -framework OpenGL
...
$(tmp_dir_engine)/%.o: $(src_dir_engine)/%.cc
	@mkdir -p $(@D)
	$(CXX) $(CXXFLAGS) -c -fPIC -o $@ $< -MMD -I$(inc_dir_engine)
...
$(tmp_dir_sandbox)/%.o: $(src_dir_sandbox)/%.cc
	@mkdir -p $(@D)
	$(CXX) $(CXXFLAGS) -c -fPIC -o $@ $< -MMD -I$(inc_dir_engine) -I$(inc_dir_sandbox)
